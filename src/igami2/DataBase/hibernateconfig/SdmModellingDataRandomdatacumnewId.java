package igami2.DataBase.hibernateconfig;
// Generated Jul 17, 2013 10:33:09 PM by Hibernate Tools 3.2.1.GA



/**
 * SdmModellingDataRandomdatacumnewId generated by hbm2java
 */
public class SdmModellingDataRandomdatacumnewId  implements java.io.Serializable {


     private int userid;
     private int searchid;
     private int globalSessionid;
     private int sdmId;
     private int indvid;

    public SdmModellingDataRandomdatacumnewId() {
    }

    public SdmModellingDataRandomdatacumnewId(int userid, int searchid, int globalSessionid, int sdmId, int indvid) {
       this.userid = userid;
       this.searchid = searchid;
       this.globalSessionid = globalSessionid;
       this.sdmId = sdmId;
       this.indvid = indvid;
    }
   
    public int getUserid() {
        return this.userid;
    }
    
    public void setUserid(int userid) {
        this.userid = userid;
    }
    public int getSearchid() {
        return this.searchid;
    }
    
    public void setSearchid(int searchid) {
        this.searchid = searchid;
    }
    public int getGlobalSessionid() {
        return this.globalSessionid;
    }
    
    public void setGlobalSessionid(int globalSessionid) {
        this.globalSessionid = globalSessionid;
    }
    public int getSdmId() {
        return this.sdmId;
    }
    
    public void setSdmId(int sdmId) {
        this.sdmId = sdmId;
    }
    public int getIndvid() {
        return this.indvid;
    }
    
    public void setIndvid(int indvid) {
        this.indvid = indvid;
    }


   public boolean equals(Object other) {
         if ( (this == other ) ) return true;
		 if ( (other == null ) ) return false;
		 if ( !(other instanceof SdmModellingDataRandomdatacumnewId) ) return false;
		 SdmModellingDataRandomdatacumnewId castOther = ( SdmModellingDataRandomdatacumnewId ) other; 
         
		 return (this.getUserid()==castOther.getUserid())
 && (this.getSearchid()==castOther.getSearchid())
 && (this.getGlobalSessionid()==castOther.getGlobalSessionid())
 && (this.getSdmId()==castOther.getSdmId())
 && (this.getIndvid()==castOther.getIndvid());
   }
   
   public int hashCode() {
         int result = 17;
         
         result = 37 * result + this.getUserid();
         result = 37 * result + this.getSearchid();
         result = 37 * result + this.getGlobalSessionid();
         result = 37 * result + this.getSdmId();
         result = 37 * result + this.getIndvid();
         return result;
   }   


}


