 ...
 * @version 1.0
 */
public class SWAT_BMPs_SoilErosionFitnessFunction implements FitnessFunction,Serializable {
    ...
    int ffchosen;
    String erosionExecFile;
    String erosionOutFile;  
	...
	/**
	  ...
	  */
	public SWAT_BMPs_SoilErosionFitnessFunction(int chosenFlag, String Fname, String OFname) {
		this.ffchosen = chosenFlag; 
                erosionExecFile = new String (Fname);
                erosionOutFile = new String (OFname);
	}
      ...
      SWAT_BMPs_Individual aIndividual = (SWAT_BMPs_Individual)individual;
       ...
       if (this.ffchosen == 1){
	    ...
	    runExec(base);
          ...
          double [] erosObjectiveValue = readObjectiveValue (base);
          
          for (int i = 0; i < (this.noOfSubbasins - this.omittedSubBasins.length); i++ ) {
            aIndividual.subbasinsFF[i][3] =  -erosObjectiveValue [i+1]; // update sub-basin fitness function #3, which is for "Sediments Reduction".
          }
          //System.out.println ("Sediment Reductions: " + erosObjectiveValue [0])  ;             
    	  return -erosObjectiveValue[0]; // negative because all objective functions are minimizing functions in the genetic algorithm
      } else{
    	  return 0.0;
      }
   }
   ...
   // To run nitrate objective function Executable model
	public void runExec(String base) throws IOException{
	     Process p;
	     ProcessBuilder pb = new ProcessBuilder(base + this.erosionExecFile);
    ...
    // Read objective function value
        public double [] readObjectiveValue (String base){
            ...
            try{             
                FileInputStream objFIS = new FileInputStream(base + this.erosionOutFile);
