 ...
 * @version 1.0
 */
public class SWAT_BMPs_PesticidesFitnessFunction implements FitnessFunction,Serializable {
    ...
    int ffchosen;
    String pesticidesExecFile;
    String pesticidesOutFile;
	...
	/**
	  ...
	  */
	public SWAT_BMPs_PesticidesFitnessFunction(int chosenFlag, String Fname, String OFname) {
		this.ffchosen = chosenFlag; 
                pesticidesExecFile = new String (Fname);
                pesticidesOutFile = new String (OFname);
   ...
   public double evaluate(Individual individual, String base) throws IOException{
      ...
      if (this.ffchosen == 1){
	    ...
	    runExec(base);
          ...
          double [] pestObjectiveValue = readObjectiveValue (base);
                    
          for (int i = 0; i < (this.noOfSubbasins - this.omittedSubBasins.length); i++ ) {
           aIndividual.subbasinsFF[i][5] =  pestObjectiveValue [i+1]; // update sub-basin fitness function #5, which is for "Pesticides Reduction".
          }  
          
    	  return pestObjectiveValue [0];
      } else{
    	  return 0.0;
      }
   }
   ...
   // To run nitrate objective function Executable model
	public void runExec(String base) throws IOException{
	     Process p;
	     ProcessBuilder pb = new ProcessBuilder(base + this.pesticidesExecFile);
    ...
    // Read objective function value
        public double [] readObjectiveValue (String base){
            ...
            try{             
                FileInputStream objFIS = new FileInputStream(base + this.pesticidesOutFile);
